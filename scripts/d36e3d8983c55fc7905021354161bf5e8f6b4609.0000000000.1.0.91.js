    ...
    e.effects = { effect: {} },
        function(e, t) {
            ...
            var n = "backgroundColor borderBottomColor borderLeftColor borderRightColor borderTopColor color columnRuleColor outlineColor textDecorationColor textEmphasisColor",
                ...
                f = e("<p>")[0],
                l, c = e.each;
            f.style.cssText = "background-color:rgba(255,255,255,0)", a.rgba = f.style.backgroundColor.indexOf("rgba") > -1, c(o, function(e, t) { t.cache = "_" + e, t.props.alpha = { idx: 3, type: "percent", def: 1 } }), s.fn = e.extend(s.prototype, {
...
(function(Modernizr, window) {
    Modernizr.addTest('csstransformspreserve3d', function() {
        ...
        Modernizr.testStyles('#modernizr{' + prop + ':' + val + ';}', function(el, rule) { computedStyle = window.getComputedStyle ? getComputedStyle(el, null).getPropertyValue(prop) : ''; });
...
(function($) {
    $.fn.clickMonitor = function(identifier, callback, insta) {
        ...
        $(this).on(insta == true ? device.instaClick : 'click', function() {
            if ($(this).hasClass('disabled') || $(this).hasClass('animating')) return false;
...
(function($) {
    $.fn.smartAnimate = function(options, time, callback, vars) {
        ...
        this.setTransition = function(tgt, options, time, callback) {
            ...
            } else { $(tgt).stop(true).animate(options, time, function() { that.setCSSTimes($(tgt), 0); if (callback) callback(tgt, vars); }); }
        };
        this.setCSSTimes = function(tgt, time) { $(tgt).css('-webkit-transition-duration', time * 0.001 + 's').css('-moz-transition-duration', time * 0.001 + 's').css('-ms-transition-duration', time * 0.001 + 's').css('-o-transition-duration', time * 0.001 + 's').css('transition-duration', time * 0.001 + 's'); };
        $(this).each(function() { that.setTransition(this, options, time, callback); });
        return this;
    }
})(jQuery);

function getTransition(left, top, rotate, scale) { return device.transforms && device.animations ? { transform: 'translate(' + left + ',' + top + ')' + (rotate != undefined ? ' rotate(' + rotate + ')' : '') + (scale != undefined ? ' scale(' + scale + ')' : '') } : $.extend((rotate != undefined ? { transform: 'rotate(' + rotate + ')' + (scale != undefined ? ' scale(' + scale + ')' : '') } : {}), { left: left, top: top }); };

function getTranslateValues(string) { return '-webkit-transform:translate(' + string + '); -moz-transform:translate(' + string + '); -ms-transform:translate(' + string + '); -o-transform:translate(' + string + '); transform:translate(' + string + ');'; };
...
function Scroller(element, options, scrollMove, scrollEnd) {
    var scroller, elID, desktop = false,
        api;
    this.init = function() {
        if (element.attr('id') == undefined) {
            ...
            uid++;
        } else { elID = element.attr('id'); }
        if (!device.touch) {
            desktop = true;
            scroller = $('#' + elID).jScrollPane({ showArrows: true, horizontalDragMaxWidth: 0 });
            api = $('#' + elID).data('jsp');
        } else { scroller = new iScroll(elID, $.extend(options, { onScrollMove: function() { scrollMove(); }, onScrollEnd: function() { scrollEnd(); } })); }
    };
    this.refresh = function() { if (desktop) { api.reinitialise(); } else { scroller.refresh(); }; };
    this.scrollTo = function(a, b) { if (desktop) { api.scrollToY(a, b); } else { scroller.scrollTo(a, b); } };
    this.init();
    scrollers.push(this);
    return this;
};
...
(function($) {
    $.fn.loadIndividualImages = function(callback) {
        $(this).each(function() { $(this).loadImage(callback, this); });
        return this;
    }
})($);
(function($) { $.fn.loadImage = function(callback, item) { $(this).loadImages(function() { callback(item); }); return this; } })($);
(function($) {
    $.fn.loadImages = function(callback, timeoutTime) {
        ...
        function addToLoaded() {
            _loaded++;
            $('.loading-bar-gauge').css({ width: ((_loaded / _total) * 100) + '%' });
...
function trk(category, action, label) {
    ...
    if (trackEnabled) { _gaq.push(['_trackEvent', category, action, label]); };
};
var app;
(function() {
    registerDevice();
    ...
    this.registerScreens = function() {
        $('#screens .screen', display).each(function(i) {
            switch ($(this).attr('id').replace('a-', '')) {
                case 'home':
                    screens['home'] = { id: total, display: $(this), screen: new ScreenHome(app, manager, $(this)) };
                    total++;
                    break;
                case 'about':
                    screens['about'] = { id: total, display: $(this), screen: new ScreenAbout(app, manager, $(this)) };
                    total++;
                    break;
                case 'skills':
                    screens['skills'] = { id: total, display: $(this), screen: new ScreenSkills(app, manager, $(this)) };
                    total++;
                    break;
                case 'work-and-play':
                    screens['work-and-play'] = { id: total, display: $(this), screen: new ScreenWorkPlay(app, manager, $(this), '#dcdcdc') };
                    total++;
                    break;
                case 'contact':
                    screens['contact'] = { id: total, display: $(this), screen: new ScreenContact(app, manager, $(this)) };
                    total++;
                    break;
                case 'awards':
                    screens['awards'] = { id: total, display: $(this), screen: new ScreenAwards(app, manager, $(this)) };
                    total++;
                    break;
                case 'login':
                    screens['login'] = { id: total, display: $(this), screen: new ScreenLogin(app, manager, $(this)) };
                    total++;
                    break;
                case '404':
                    screens['404'] = { id: total, display: $(this), screen: new Screen404(app, manager, $(this)) };
                    total++;
                    break;
                default:
                    break;
...
function AppHeaderFooter(app, display) {
    ...
    this.prepareDisplay = function() {
        ...
        $('.f-on', display).smartAnimate({ opacity: 0 }, 0);
        $('.f-off', display).smartAnimate({ opacity: 1 }, 0);
    };
    this.addListeners = function() {
        $('.go-footer-info', display).click(function() { that.showFooter(); return false; });
        $('.go-close-footer-info', display).click(function() { that.hideFooter(); return false; });
        $('body').on(device.over, '.flp-btn', that.itemOver).on(device.out, '.flp-btn', that.itemOut);
        $('.flp-btn div').smartAnimate({ top: '0px' }, 0);
    };
    this.itemOver = function() {
        if ($(this).data('disabled') != true) { $('> div', this).smartAnimate(($(this).hasClass('flp-left') ? getTransition((-$('> div', this).width() * 0.5) + 'px', '0px') : $(this).hasClass('flp-right') ? getTransition(($('> div', this).width() * 0.5) + 'px', '0px') : getTransition('0px', (-$('> div', this).height() * 0.5) + 'px')), device.touch ? 0 : 200); }
    };
    this.itemOut = function() {
        if ($(this).data('disabled') != true) { if ($(this).data('outDisabled') != true) { $('div', this).smartAnimate(getTransition('0px', '0px'), 200); } }
    };
    this.showFooter = function() {
        $('#footer-bar', display).css({ height: '64px' });
        $('#footer-bar-display', display).smartAnimate(getTransition('0px', '0%'), 500);
        $('.f-off', display).smartAnimate({ opacity: 0 }, 0);
        $('.f-off', display).hide();
        $('.f-on', display).show().smartAnimate({ opacity: 0 }, 0, function() { $('.f-on', display).smartAnimate({ opacity: 1 }, 500); });
    };
    this.hideFooter = function() {
        $('#footer-bar-display', display).smartAnimate(getTransition('0px', '100%'), 500, function() { $('#footer-bar', display).css({ height: '0px' }); });
        $('.f-on', display).smartAnimate({ opacity: 0 }, 0);
        $('.f-on', display).hide();
        $('.f-off', display).show().smartAnimate({ opacity: 1 }, 500);
    };
    this.init();
};
...
function AppManager(app, display, screens) {
    ...
    var key = { active: false },
        targetScreen, resizeTimeout, deepLink = { value: '', changed: false },
        pageInit = { currPage: currPage, used: false };
    var historyData = { state: window.history.pushState !== undefined, titleString: '' };
    ...
    this.deepLinking = function() { this.html5DeepLink(); };
    ...
    this.camelCase = function(str) { return str.toLowerCase().replace(/(^| )(\w)/g, function(x) { return x.toUpperCase(); }); };
    this.monitorDimensions = function() {
        $(window).bind('resize.app', function() { that.updateDimensions(); });
        $(window).trigger('resize.app');
    };
    this.addListeners = function() { $(document).keydown(that.monitorKeyPress).keyup(that.monitorKeyRelease); };
    this.monitorKeyPress = function(e) { key.active = false; if (e.shiftKey == true) key.active = true; if (e.metaKey == true) key.active = true; };
    this.monitorKeyRelease = function(e) { key.active = false; };
    this.checkLinkSelect = function(value) { if (screens[value] != undefined) { that.viewScreen(value); return false; } };
    this.updateDimensions = function() {
        that.goRefreshDimensions();
        if (device.touch) {
            clearTimeout(resizeTimeout);
            resizeTimeout = setTimeout(function() { that.goRefreshDimensions(); }, 100);
            resizeTimeout = setTimeout(function() { that.goRefreshDimensions(); }, 500);
        };
    };
    this.goRefreshDimensions = function() {
        if (this.currentScreen != null) {
            screens[this.currentScreen].display.css({ minHeight: $(window).height() });
    ...
    this.viewScreen = function(screen, linkData) {
        ...
        if (screens[screen] == undefined) screen = 'skills';
        $('.sec-loader').hide();
        trk('Sections', 'View Section', screen);
        this.updateAddressTitle(linkData, screen);
        this.updateLinkStates(screen);
        if (this.currentScreen == screen) { screens[currentScreen].screen.readLinkData(linkData); return false; }
    ...
    this.closeScreen = function(currentScreen, screen, linkData) {
        if (currentScreen == null) { this.openScreen(screen, linkData); return false; }
        targetScreen = screen;
        screens[currentScreen].screen.remove(function() { screens[currentScreen].display.hide(); if (targetScreen == screen) that.openScreen(screen, linkData); });
    };
    ...
    this.setBackgroundColor = function(color) { $('body').smartAnimate({ backgroundColor: color }, 300); };
};
...
function ScreenHome(app, manager, display) {
    var that = this,
        ...
        manager = manager,
        display = display,
        loadStatus = { loaded: false, loading: false },
        status = { adding: false, removing: false, enabled: false };
    var xmlPath = '/xml/letters.xml',
        json;
    var statements;
    this.init = function() { statements = new SmartStatements($('#home-statements', display), display, that); };
    this.loadAssets = function() {
        ...
        $.ajax({
            ...
            complete: function(data) {
                ...
                $('.sec-loader').show().find('.loading-bar-gauge').css({ width: '0%' });
    ...
    this.beginDisplay = function() {
        if (loadStatus.loading) { loadStatus.loading = false; }
        status.enabled = true;
        statements.setStatement(0);
        $(window).trigger('resize.app');
    };
    this.updateDimensions = function() { statements.updatePositions(); };
    this.setBackgroundColor = function(color) {
        if (!status.enabled) return false;
        manager.setBackgroundColor(color);
    };
    this.getHeight = function() { return 100; };
    this.add = function() {
        manager.setBackgroundColor('#ffc300;');
        if (status.adding) return false;
        status.adding = true;
        status.removing = false;
        if (!loadStatus.loaded) { this.loadAssets(); } else { this.beginDisplay(); }
    };
    this.remove = function(callback) {
        if (status.removing) return false;
        status.adding = false;
        status.enabled = false;
        status.removing = true;
        if (loadStatus.loaded) { statements.closeStatements(callback); } else { callback(); }
    };
...
function SmartStatements(display, screen, topLevel) {
    var display = display,
        ...
        that = this,
        topLevel = topLevel,
        json, tmp, loadDiv = { string: '' },
        currentStatement = null,
        statement, statements = { statement: [], current: null, item: null, time: 10000, timeout: null, randomizer: { time: 1000, timeout: null } },
        letters = { title: { a: [], r: [], prefix: null, format: null, letterHeight: 0, spaceWidth: 0 }, description: { a: [], r: [], prefix: null, format: null, letterHeight: 0, spaceWidth: 0 } },
        letterTime = { interval: null, count: 0, timeout: 2000 },
        bullets = { width: 0, height: 0, prefix: null, format: null, bullet: [], raw: '' },
        shifting = false,
    ...
    var sideIndex = 0,
        sideIndexGroup = 2,
        sidesStd = [{ s: 'dl', d: 'left:100%', db: { left: '100%', top: '0%' }, o: { left: '-100%', top: '0%' } }, { s: 'dr', d: 'left:-100%', db: { left: '-100%', top: '0%' }, o: { left: '100%', top: '0%' } }, { s: 'dt', d: 'top:100%', db: { left: '0%', top: '100%' }, o: { left: '0%', top: '-100%' } }, { s: 'db', d: 'top:-100%', db: { left: '0%', top: '-100%' }, o: { left: '0%', top: '100%' } }],
        sidesHw = [{ s: 'dl', d: getTranslateValues('100%,0%'), db: { transform: 'translate(100%,0%)' }, o: { transform: 'translate(-100%,0%)' } }, { s: 'dr', d: getTranslateValues('-100%,0%'), db: { transform: 'translate(-100%,0%)' }, o: { transform: 'translate(100%,0%)' } }, { s: 'dt', d: getTranslateValues('0%,100%'), db: { transform: 'translate(0%,100%)' }, o: { transform: 'translate(0%,-100%)' } }, { s: 'db', d: getTranslateValues('0%,-100%'), db: { transform: 'translate(0%,-100%)' }, o: { transform: 'translate(0%,100%)' } }],
        showStd = { top: '0px', left: '0px' },
        showHw = { transform: 'translate(0%,0%)' },
        show, sides;
    ...
    this.compileImages = function() { loadDiv.string = device.retina ? '<img src="/images/lettering/learn-more.png" />' : '<img src="/images/lettering/learn-more@2x.png" />'; };
    ...
    this.compileLetters = function(letters, data, lightScale) {
        ...
        while (i--) {
            item = data.l[i];
            itemData = { used: false, img: item.img, width: (device.mobile ? Math.floor(parseInt(item.width) * (lightScale ? 0.7 : 0.5)) : parseInt(item.width)), height: letters.letterHeight, char: item.Text };
    ...
    this.compileStatements = function() {
        ...
        for (var i = 0, il = $('.statement-spinner', display).length; i < il; i++) {
            ...
            statements.statement.push({ id: i, added: false, color: $(item).css('color'), container: $(item), title: $('.statement .st-title', item).text().toLowerCase().split(' '), description: $('.statement .st-desc', item).text().toLowerCase().split(' '), parsed: false, titleContent: null, descriptionContent: null, letters: { t: [], d: [] }, minWidth: 0 });
        ...
        if (!device.touch) $('.st-bullet', display).bind(device.over, this.bulletOver).bind(device.out, this.bulletOut);
    ...
    this.bulletOver = function() { $('.bullet', this).addClass('bullet-hover'); };
    this.bulletOut = function() { $('.bullet', this).removeClass('bullet-hover'); };
    ...
    this.setStatement = function(id) {
        if (statements.statement.length == 0) return;
        if (id > statements.statement.length - 1) id = 0;
        disabled = false;
        clearTimeout(statements.timeout);
        if (currentStatement != null) {
            if (currentStatement.id == id) return false;
            shifting = true;
    ...
    this.closeStatement = function(id, forward, callback) {
        ...
        $(currentStatement.container).smartAnimate({ opacity: 0 }, 300, function(tgt) { $(tgt).hide(); if (callback) { callback(); return false; } });
        $('.statement .st-bullets', currentStatement.container).css({ visibility: 'hidden' });
        if (callback == undefined) this.addStatement(id, forward);
    };
    this.addStatement = function(id, forward) {
        ...
        $('.statement .st-bullets', currentStatement.container).css({ visibility: 'visible' });
        $('.statement .st-bullets', currentStatement.container).smartAnimate({ opacity: 0 }, 0, function() { $('.statement .st-bullets', currentStatement.container).smartAnimate({ opacity: 1 }, 300); });
        $(window).trigger('resize.app');
        tmpTransform.out = device.mobile ? { opacity: 1 } : { transform: 'rotate(' + (forward ? 10 : -10) + 'deg)', opacity: 1 };
        $(currentStatement.container).smartAnimate(tmpTransform.out, 0, function() {
            topLevel.setBackgroundColor(currentStatement.color);
            $(currentStatement.container).smartAnimate({ transform: 'rotate(0deg)' }, 500, function() { shifting = false; });
            that.revealStatement();
        });
        this.updateBullets(id);
    };
    this.updatePositions = function() {
        $('#home-statements').css({ left: $(window).width * 0.5 + 'px' });
        if (currentStatement != null) {
            $('.statement', currentStatement.container).css({ width: $(screen).width() + 'px', left: -Math.max(currentStatement.minWidth * 0.5, $(screen).width() * 0.5) + 'px' });
            $(screen).css({ minHeight: Math.max($(window).height(), $('.statement', currentStatement.container).height() + $('#header').outerHeight(true) + 80) + 'px' });
            $('.statement', currentStatement.container).css({ bottom: ($(screen).height() * 0.5 - $('.statement', currentStatement.container).height() * 0.5) + 'px' });
        };
    };
    ...
    this.runTimer = function() {
        ...
        statements.timeout = setTimeout(function() {
            ...
            if (currentStatement == null) { that.setStatement(0); return false; }
            that.setStatement(currentStatement.id + 1);
        }, statements.time);
    };
    this.letterRandomizer = function() {
        clearTimeout(statements.randomizer.timeout);
        if ($(currentStatement.letters.t).length == 0) return false;
        this.letterOver($(currentStatement.letters.t[Math.floor(Math.random() * currentStatement.letters.t.length)]).parent());
        statements.randomizer.timeout = setTimeout(function() { that.letterRandomizer(); }, statements.randomizer.time);
    };
    this.stopLetterRandomizer = function() { clearTimeout(statements.randomizer.timeout); };
    this.addStatementListeners = function() {
        $('.st-title .l', currentStatement.container).bind(device.over, function() { that.letterOver($(this)); });
        $('.st-desc .w', currentStatement.container).bind(device.over, function() { that.descLetterOver($(this)); });
    };
    this.letterOver = function(letter) {
        if (shifting || currentStatement == null) return false;
        clearTimeout($('.l-c', letter).data('letterTimeout'));
        $('.l-c', letter).smartAnimate(sides[parseInt($('.ndx', letter).text())].o, 300);
        var timeout = $('.l-c', letter).data({ letterTimeout: setTimeout(function() { that.letterOut(letter); }, letterTime.timeout) });
    };
    this.descLetterOver = function(letter) {
        ...
        var timeout = $('.w-m', letter).data({ letterTimeout: setTimeout(function() { that.descLetterOut(letter); }, letterTime.timeout) });
    };
    this.letterOut = function(letter) { $('.l-c', letter).smartAnimate(show, 300); };
    this.descLetterOut = function(letter) { $('.w-m', letter).smartAnimate(show, 300); };
    this.parseStatement = function(id) {
        ...
        tmp.statement = { titleRaw: '', descriptionRaw: '', titleWidths: [], descriptionWidths: [] };
        for (i = 0, il = statement.title.length; i < il; i++) {
            ...
            if (tmp.data.width + letters.title.spaceWidth > statement.minWidth) statement.minWidth = tmp.data.width + letters.title.spaceWidth;
        ...
        $(statement.descriptionContent).css({ maxWidth: tmp.totalTitleWidth + 'px' });
        $('#container').css({ minWidth: statement.minWidth + 'px' });
        $('.statement', statement.container).css({ minWidth: statement.minWidth + 'px' });
        statement.parsed = true;
    };
    this.parseWord = function(l, letters, last, grouped) {
        tmp.word = { raw: '<div class="w">', innerRaw: '', width: 0 };
        tmp.item = [];
    ...
    this.createLetter = function(item, letters, grouped, sideIndexGroup) {
        if (grouped) { return { raw: '<div class="l" style="width:' + item.width + 'px; height:' + (item.height) + 'px; background-image:url(' + (letters.prefix + item.img + letters.format) + '); background-size:' + item.width + 'px ' + item.height + 'px"></div>', width: item.width }; } else {
            sideIndex = Math.floor(Math.random() * sides.length);
            side = sides[sideIndex];
            return { raw: '<div class="l" style="width:' + item.width + 'px; height:' + (item.height * 0.5) + 'px;"><div class="l-c" style="' + side.d + ';"><span class="ndx">' + sideIndex + '</span><div class="l-i l-i-b l-i-b-' + side.s + '" style="background-image:url(' + (letters.prefix + item.img + letters.format) + '); background-size:' + item.width + 'px ' + item.height + 'px; background-position:0 -' + (item.height * 0.5) + 'px"></div><div class="l-i" style="background-image:url(' + (letters.prefix + item.img + letters.format) + '); background-size:' + item.width + 'px ' + item.height + 'px"></div></div></div>', width: item.width };
        }
    };
};
...
function ScreenSkills(app, manager, display) {
    var that = this,
        ...
        loadStatus = { loaded: false, loading: false },
        status = { adding: false, removing: false, enabled: false },
        map = { maxWidth: 0, minHeight: 300, maxHeight: 700, availableWidth: 0, realWidth: 0, dimension: [] },
        solar = { maxWidth: 0, img: { origWidth: 0, origHeight: 0 } },
        solarInfo = { offsetLeft: 0, offsetTop: 0, planetWidth: 0, planetHeight: 0 },
        linkIndex = 0,
        mapHeight = 0,
        primaryView = '';
    currentView = 'solar', planetHoverDisabled = false, tmp = [], planets = { timeout: null, time: 100 }, dist = {}, planetAnimator = { interval: null, time: 20, planets: [] }, closeTimeout = null, data = { display: { solar: $('<div></div>'), info: $('<div></div>'), planet: $('<div></div>'), option: '' }, template: { solar: $('#map-templates .tmp-solar', display), info: $('#map-templates .tmp-info', display), option: $('#map-templates .tmp-option', display).html(), planet: $('#map-templates .tmp-planet', display) } };
    var imageList = { string: '' },
        preloadImgs = [{ std: '/images/screens/skills/map/title.png', retina: '/images/screens/skills/map/title@2x.png' }, { std: '/images/screens/skills/map/planetview/planet-ring.png', retina: '/images/screens/skills/map/planetview/planet-ring@2x.png' }, { std: '/images/screens/skills/map/planets/sun/sun-glow.png', retina: '/images/screens/skills/map/planets/sun/sun-glow@2x.png' }, { std: '/images/screens/skills/map/info-marker.png', retina: '/images/screens/skills/map/info-marker@2x.png' }, { std: '/images/screens/skills/map/planets/sun/small-planet.png', retina: '/images/screens/skills/map/planets/sun/small-planet@2x.png' }, { std: '/images/screens/skills/map/planet-ring.png', retina: '/images/screens/skills/map/planet-ring@2x.png' }, { std: '/images/ui/map/buttons/button-blue/button-left.png', retina: '/images/ui/map/buttons/button-blue/button-left@2x.png' }, { std: '/images/ui/map/buttons/button-blue/button-right.png', retina: '/images/ui/map/buttons/button-blue/button-right@2x.png' }, { std: '/images/ui/map/buttons/button-blue/button-bar.png', retina: '/images/ui/map/buttons/button-blue/button-bar@2x.png' }, { std: '/images/ui/map/buttons/button-white/button-left.png', retina: '/images/ui/map/buttons/button-white/button-left@2x.png' }, { std: '/images/ui/map/buttons/button-white/button-right.png', retina: '/images/ui/map/buttons/button-white/button-right@2x.png' }, { std: '/images/ui/map/buttons/button-white/button-bar.png', retina: '/images/ui/map/buttons/button-white/button-bar@2x.png' }];
    this.init = function() {
        this.checkSupport();
        this.compileImages();
        this.addPlanets();
    ...
    this.compileImages = function() {
        this.compileImages = function() { imageList.string = ''; for (var i = 0, il = preloadImgs.length; i < il; i++) { imageList.string += '<img src="' + (device.retina ? preloadImgs[i].retina : preloadImgs[i].std) + '" />' }; };
    };
    this.addPlanets = function() {
        $('#map-data .sec', display).each(function(index) {
            tmp = [];
            tmp.data = { title: $('.s-title', this).html(), info: $('.s-info', this).html(), pos: $('.s-pos', this).text(), size: Number($('.s-size', this).text()), time: $('.s-orbit-time', this).text(), className: $('.s-class', this).text(), planet: { texture: $('.s-texture', this).text(), std: $('.s-planet .1x', this).text(), retina: $('.s-planet .2x', this).text() } };
            ...
            $('.planet-spinner', tmp.info).css({ backgroundImage: 'url(' + tmp.data.planet.texture + ')' });
            ...
            planetAnimator.planets.push({ id: index, time: tmp.data.time, angle: 0, radius: Math.abs(parseInt($('.s-pos', this).text(), 10)) - 50, radian: 0, speed: parseInt($('.s-orbit-time', this).text(), 10) });
        });
    ...
    this.prepareDisplay = function() {
        ...
        $('#map-solar-system #solar-display', display).css({ maxWidth: solar.maxWidth + 'px' });
    };
    this.addListeners = function() {
        if (!device.touch) {
        ...
        } else { $('#map-solar-system', display).click(that.checkClosestPlanet); };
    ...
    this.checkClosestPlanet = function(e, callback) {
        ...
        $('.solar-system .planetoid', display).each(function() {
            ...
            pl.tmpDist = that.getDistance({ x: pl.x, y: pl.y }, { x: pl.tmpX, y: pl.tmpY });
            if (pl.tmpDist < pl.dist) {
                pl.dist = pl.tmpDist;
                pl.item = $(this).closest('.orbit');
            };
        });
        if (pl.dist < 60) { if (callback != undefined) { callback(); } else { that.viewPlanet(getClassSuffix('view-planet-', $('.pos-block', pl.item).attr('class'))); } };
    };
    ...
    this.planetLinkOut = function() { that.planetOut(); };
    this.planetOver = function(e) {
        if (planetHoverDisabled) return false;
        ...
        $('.planet-info .planet-rect', display).css({ width: solarInfo.planetWidth, height: solarInfo.planetHeight, top: -(solarInfo.planetHeight * 0.5), left: -(solarInfo.planetWidth * 0.5) });
        $('.planet-info .info-title', display).text($('.planet-data .planet-title', item).text());
        $('.planet-info', display).css({ top: solarInfo.offsetTop, left: solarInfo.offsetLeft });
        $('#map-solar-system .planet-rect', display).bind(device.out, function() { that.planetOut(); });
        $('.planet-info', display).show();
        $('.planet-info .info-display', display).css({ top: -($('.planet-info .info-display', display).height()) });
        $('.planet-info ', display).smartAnimate({ opacity: 0, transform: 'translateZ(3000px) scale(0.9) rotate(' + (solarInfo.direction ? 40 : -40) + 'deg)' }, 0, function() { $('.planet-info ', display).smartAnimate({ opacity: 1, transform: 'translateZ(3000px) scale(1) rotate(0deg)' }, 300); });
    };
    this.planetOut = function() {
        $('#map-solar-system .planet-rect', display).unbind(device.out);
        $('.planet-info', display).smartAnimate({ opacity: 0 }, 0, function() { $('.planet-info', display).hide(); });
        if (solarInfo.planet != null) solarInfo.planet.removeClass('planet-paused');
    };
    this.planetItemOver = function() { $('.planet-item-thumb', this).show().smartAnimate({ opacity: 0, transform: 'rotate(60deg)' }, 0, function(tgt) { $(tgt).smartAnimate({ opacity: 1, transform: 'rotate(0deg)' }, 300); }); };
    this.planetItemOut = function() { $('.planet-item-thumb', this).smartAnimate({ opacity: 0, transform: 'rotate(60deg)' }, 300); };
    this.checkInfoClick = function() { that.viewPlanet($(this).closest('.planet-info').find('.planet-class').text()); };
    ...
    this.beginDisplay = function() {
        ...
        $('#container').css({ minWidth: $(display).css('min-width') });
        $('#map-display', display).show();
        $('#map-solar-system', display).show().css({ visibility: 'hidden' });
    ...
    that.updatePlanetPositions = function() {
        $(planetAnimator.planets).each(function(id) {
            ...
            if ($(this.planet).hasClass('planet-paused')) {} else {
                ...
                this.angle %= 360;
                $('.pos', this.planet).css({ left: this.x + '%', top: this.y + 'px' });
            }
        });
    };
    this.deg2rad = function(degree) { return degree * (Math.PI / 180); };
    this.setViewSolar = function() {
        planetHoverDisabled = true;
        primaryView = 'solar';
        currentView = 'solar';
        $('#map-display', display).removeClass('planet-view');
        $('#map-planet-view .planet-element .planet-abs').smartAnimate({ opacity: 0 }, 0);
        $('#map-overlays-inner', display).smartAnimate($.extend(getTransition(0, '100px'), { opacity: 0 }), 200, function() {
            ...
            $('#map-planet-view', display).hide();
            $('#map-overlays-inner', display).smartAnimate($.extend(getTransition(0, 0), { opacity: 1 }), 100);
            $('#map-solar-system', display).show().css({ visibility: 'visible' }).smartAnimate({ opacity: 1 }, 0);
            $('#map-solar-system #solar-display', display).smartAnimate($.extend({ transform: 'scale(0.1)' }, { opacity: 0 }), 0);
            $('#map-solar-system #map-title-display', display).smartAnimate(getTransition(0, '100%'), 0, function() {
                $('#map-solar-system #map-title-display', display).smartAnimate(getTransition(0, '0%'), 300, function() {
                    ...
                    $('#map-solar-system #solar-display', display).smartAnimate($.extend({ transform: 'scale(1)' }, { opacity: 1 }), 500, function() { planetHoverDisabled = false; });
                    $(window).trigger('resize.app');
                });
            });
            $(window).trigger('resize.app');
    ...
    this.setViewPlanet = function(planet) {
        ...
        $('#map-overlays-inner', display).smartAnimate($.extend(getTransition(0, '100px'), { opacity: 0 }), 200, function() {
            ...
            $('#map-planets-view', display).hide();
            $('#map-overlays-inner', display).smartAnimate($.extend(getTransition(0, 0), { opacity: 1 }), 300);
            $('#map-planet-view', display).show();
            $('#map-planet-view .planet-' + planet + ' .planet-abs', display).smartAnimate({ opacity: 0 }, 0);
            $('#map-planet-view', display).css({ visibility: 'visible' }).smartAnimate({ opacity: 0 }, 0, function() {
                $(window).trigger('resize.app');
                $('#map-planet-view', display).smartAnimate({ opacity: 1 }, 700);
                $('#map-planet-view .planet-abs', display).smartAnimate({ opacity: 0 }, device.mobile ? 750 : 400, function() { $('#map-planet-view .planet-abs', display).smartAnimate({ opacity: 1 }, 500); });
            });
        });
        $('html, body').animate({ scrollTop: 1 }, 300);
        $(window).trigger('resize.app');
    ...
    this.updateDimensions = function() {
        if (currentView == 'solar') this.updateSolarView();
        map.availableWidth = $(display).width() - 40;
        map.realWidth = Math.min(map.maxWidth, map.availableWidth);
        $('#map', display).width(map.realWidth).css({ left: Math.floor(-map.realWidth * 0.5) + 'px' });
        ...
        $('#map-overlays', display).css({ top: $('#map-nav-top', display).outerHeight(true) });
        $('#map-overlays-inner', display).css({ top: 0, paddingBottom: currentView == 'planets' ? '0px' : $('#map-nav-bottom', display).outerHeight(true) + 20 });
        map.dimension.height = Math.max($(window).height(), mapHeight + $('#header').outerHeight(true) + 100);
        map.dimension.center = -mapHeight * 0.5;
        $('#map', display).css({ top: map.dimension.center + 'px' });
        $(display).css({ height: map.dimension.height + 'px' });
        if (currentView == 'solar') this.updateSolarView();
        if (currentView == 'planets') this.updatePlanetsView();
        $('#map-planet-view', display).css({ top: ((mapHeight) - (($('#map-nav-top', display).outerHeight(true) + $('#map-overlays-inner', display).outerHeight(true)) - $('#map-overlays-inner', display).height())) * 0.5 - ($('#map-planet-view', display).height() * 0.5) });
    };
    this.updateSolarView = function() {
        ...
        $('.solar-base', display).css({ width: solar.img.newWidth, height: '100%' });
        $('#map-solar-system', display).height(solar.img.newHeight);
        $('#map-solar-system', display).css({ top: ((mapHeight) - (($('#map-nav-top', display).outerHeight(true) + $('#map-overlays-inner', display).outerHeight(true)) - $('#map-overlays-inner', display).height())) * 0.5 - (solar.img.newHeight * 0.5) });
    };
    this.updatePlanetsView = function() { $('#map-planets-view', display).css({ top: (mapHeight - (($('#map-nav-top', display).outerHeight(true) + $('#map-overlays-inner', display).outerHeight(true)) - $('#map-overlays-inner', display).height())) * 0.5 - ($('#map-planets-view', display).height() * 0.5) }); };
    this.removeDisplay = function(callback) {
        if (currentView == 'solar') {
            $('#map-solar-system #solar-display', display).smartAnimate($.extend({ transform: 'scale(0.1)' }, { opacity: 0 }), 300);
            ...
            clearTimeout(closeTimeout);
            closeTimeout = setTimeout(function() { callback(); }, 300);
        } else { callback(); };
    };
    this.add = function() {
...
function ScreenAbout(app, manager, display) {
    ...
    this.updateDimensions = function() {
        ...
        $(display).css({ height: Math.max($(window).height(), displayHeight + $('#header').outerHeight(true) + 150) });
    };
    this.add = function() {
...
function HeadSpin(display, screen, topLevel, imageList) {
    var display = display,
        ...
        transitions = [],
        head = { overlays: [], segments: [], segment: { segString: '' }, baseString: '', compiled: false, randomizer: { timeout: null, time: 2000 } },
        dm = { availableWidth: 0, availableHeight: 0 },
        headEnabled = false,
        headStarted = false,
        timeouts = { openTimeout: null },
    ...
    this.createSegments = function() {
        ...
        for (var i = 0, il = head.numSegments; i < il; i++) {
            head.segments.push({ width: json.head[0].segment[i].width, timeout: null });
    ...
    this.createOverlays = function() { for (var i = 0, il = json.head[0].overlay.length; i < il; i++) { head.overlays.push(head.path + json.head[0].overlay[i].src); }; };
    ...
    this.addListeners = function() {
        ...
        $('#head-spin .explore-btn', display).click(function() { that.goExplore(); return false; });
    };
    this.resetDisplay = function() {
        ...
        $('#head-left #head-left-shift', display).smartAnimate($.extend(getTransition('0%', '0%', '0deg'), { opacity: 1 }), 0);
        $('#head-right #head-right-shift', display).smartAnimate($.extend(getTransition('0%', '0%', '0deg'), { opacity: 1 }), 0);
    ...
    this.removeDisplay = function(callback) {
        ...
        $('#head-right .head-base-overlay, #head-right .head-segments', display).smartAnimate(getTransition('-100%', 0), 300, function() { callback(); });
    };
    ...
    this.stopHead = function() { this.stopHeadRandomizer(); };
    ...
    this.flipSegment = function(segment, left, instant, id, rapid) {
        ...
        $('.head-seg-overlays', segment).smartAnimate(getTransition((left ? (head.segments[$(segment).index()].width) : '-' + (head.segments[$(segment).index()].width)), 0), (instant == true ? 0 : rapid ? 300 : 600), function() {
            ...
            setTimeout(function() { $('.head-seg-overlays', segment).smartAnimate(getTransition('0%', 0), rapid ? 200 : 400); }, 80);
        });
    };
    ...
    this.splitHead = function() {
        ...
        $('#head-left #head-left-shift', display).smartAnimate($.extend(getTransition('-100%', '10%', '-10deg'), { opacity: 0 }), 1000);
        $('#head-right #head-right-shift', display).smartAnimate($.extend(getTransition('100%', '10%', '10deg'), { opacity: 0 }), 1000, function() { $(display).hide(); });
    };
    this.hideSegments = function(side, left, instant) { $('.head-seg-overlays', side).smartAnimate(getTransition((left ? '100%' : '-100%'), 0), instant ? 0 : 600); };
    ...
    this.headRandomizer = function() {
        ...
        head.randomizer.timeout = setInterval(function() {
            while (head.randomizer.index == head.randomizer.previous) head.randomizer.index = Math.floor(Math.random() * $('.head-seg', display).length);
    ...
    this.stopHeadRandomizer = function() { clearInterval(head.randomizer.timeout); };
    this.updateDimensions = function() {
        if (!head.compiled) return false;
        dm.availableWidth = $(screen).width() - screenPad;
        dm.availableHeight = $(window).height() - $('#header').outerHeight() - 150;
        this.setHeadDimension();
        $(display).css({ minWidth: (dm.width + screenPad) + 'px', minHeight: Math.max(dm.availableHeight, dm.height + $('#header').outerHeight() + 150) });
        $('#head-spin', display).css({ width: dm.width + 'px', height: dm.height + 'px', top: (-dm.height * 0.5) + 'px', left: (-dm.width * 0.5) + 'px' });
        $('#head-left .head-seg', display).height(Math.round(dm.height / head.numSegments)).each(function(i) { $('.h-s-overlay', this).css({ top: (-Math.round(dm.height / head.numSegments) * i) + 'px' }); });
        $('#head-right .head-seg', display).height(Math.round(dm.height / head.numSegments)).each(function(i) { $('.h-s-overlay', this).css({ top: (-Math.round(dm.height / head.numSegments) * i) + 'px' }); });
        $('.head-seg-overlays, .head-base-overlay, .head-title-overlay', display).css({ width: dm.width + 'px', height: dm.height + 'px' });
    };
...
function Anatomy(display, screen, topLevel, imageList) {
    var display = display,
        ...
        json, tmp = [],
        screenPad = 40,
        anatomy = { compiled: false, section: [], view: [], columns: 0, tmpColumns: 0, startPos: { x: 0, y: 0 }, openTimeout: null, openFinishedTimeout: null, openFinishedTime: 0, sectionsEnabled: false, hideTimeout: null, hideTime: 0, currentView: 0 },
        dm = { availableWidth: 0, availableHeight: 0 },
        targetSection = null,
        otherSections = null;
    ...
    this.compileData = function() { anatomy = $.extend(anatomy, { maxColumns: Number(json.maxcolumns), sectionMinSize: Number(json.sectionminsize), sectionMaxSize: Number(json.sectionmaxsize), maxWidth: Number(json.maxwidth), path: (device.retina ? json.retinaprefix : json.prefix) }); };
    this.compileSections = function() {
        ...
        for (var i = 0, il = json.section.length; i < il; i++) {
            ...
            switch (json.section[i].id) {
                case 'love':
                    tmp.f = true;
                    anatomy.view.push({ id: i, item: new AnatomyLove(display, $('#an-view-love', display), screen, that, json.section[i], anatomy.path, this.imageList) });
                    break;
                case 'core':
                    tmp.f = true;
                    anatomy.view.push({ id: i, item: new AnatomyCore(display, $('#an-view-core', display), screen, that, json.section[i], anatomy.path, this.imageList) });;
                    break;
                case 'work':
                    tmp.f = true;
                    anatomy.view.push({ id: i, item: new AnatomyWork(display, $('#an-view-work', display), screen, that, json.section[i], anatomy.path, this.imageList) });;
                    break;
            };
            if (tmp.f) anatomy.section.push({ id: i, item: new AnatomySection(display, screen, that, json.section[i], json.numsegments, anatomy.path, this.imageList) });
        };
        anatomy.totalSections = anatomy.section.length;
    };
    this.addListeners = function() {
        $('.an-section-hit', display).bind(device.over, that.sectionOver);
        if (!device.touch) $('.an-section-container', display).bind(device.out, function() {
            if (!anatomy.sectionsEnabled) return false;
            that.deselectOtherSections(99);
        });
        $('.an-examine-hit', display).bind(device.instaClick, function() { that.viewSection($(this).closest('.an-section').index()); return false; });
        $('.close-view', display).click(function() { that.closeSection(); return false; });
    };
    this.sectionOver = function() { that.setActiveSection($(this).closest('.an-section').index()); };
    this.setActiveSection = function(id) {
        ...
        $('.an-section-display .section-seg .section-seg-overlay', targetSection).each(function(i) {
            $(this).smartAnimate(getTransition((i % 2 ? '100%' : '-100%'), 0), 300, function() {
                if (i == $('.an-section-display .section-seg .section-seg-overlay', targetSection).length - 1) { $('.an-examine-hit', targetSection).show(); };
            });
        });
    };
    this.deselectOtherSections = function(id) {
        ...
        otherSections.each(function() {
            if ($(this).data('closed') == true) {} else {
                $(this).data('closed', true);
                $('.an-section-hit', this).hide();
                $('.an-examine-hit', this).hide();
                $('.an-section-display .section-seg .section-seg-overlay', this).each(function(i) { $(this).smartAnimate(getTransition('0%', 0), 300); });
    ...
    this.hideSections = function(callback) {
        ...
        otherSections.each(function() {
            $('.an-section-hit', this).hide();
            $('.an-examine-hit', this).hide();
            $('.an-section-display .section-seg .section-seg-overlay', this).each(function(i) { $(this).smartAnimate(getTransition(i % 2 ? '-100%' : '100%', 0), anatomy.hideTime); });
        });
        clearTimeout(anatomy.hideTimeout);
        anatomy.hideTimeout = setTimeout(function() { $('#anatomy-sections', display).hide(); if (callback) callback(); }, anatomy.hideTime);
    };
    this.showSections = function() {
        ...
        otherSections.each(function() {
            ...
            $('.an-section-display .section-seg .section-seg-overlay', this).each(function(i) {
                $(this).smartAnimate(getTransition(i % 2 ? '-100%' : '100%', 0), 0, function(tgt) { $(tgt).smartAnimate(getTransition('0%', 0), 400); });
            });
        });
        clearTimeout(anatomy.hideTimeout);
    ...
    this.showViews = function(id) {
        ...
        $('#anatomy-views-display', display).smartAnimate($.extend(getTransition(0, '300px'), { opacity: 0 }), 0, function() { $('#anatomy-views-display', display).smartAnimate($.extend(getTransition(0, '0px'), { opacity: 1 }), 300); });
    };
    this.hideViews = function(callback) { $('#anatomy-views-display', display).smartAnimate($.extend(getTransition(0, '300px'), { opacity: 0 }), 300, function() { $('#anatomy-views', display).hide(); if (callback) callback(); }); };
    ...
    this.resetDisplay = function() {
        ...
        $('.an-section', display).each(function() {
            ...
            $('.an-section-display', this).smartAnimate($.extend(getTransition(anatomy.startPos.x + 'px', anatomy.startPos.y + 'px', undefined, 0.5), { opacity: 0 }), 0);
        });
    };
    this.hide = function() { $(display).hide(); };
    this.removeDisplay = function(callback) {
        switch (this.currentDisplay) {
            case 'sections':
    ...
    this.startAnatomy = function() {
        ...
        anatomy.openTimeout = setTimeout(function() {
            $('.an-section-display', display).each(function() { $(this).smartAnimate($.extend(getTransition('0%', '0px', undefined, 1), { opacity: 1 }), 1000); });
        }, 100);
        anatomy.openFinishedTimeout = setTimeout(function() { $('.an-section-hit', display).show(); }, anatomy.openFinishedTime);
    };
    this.updateDimensions = function() {
        if (!anatomy.compiled) return false;
        dm.availableWidth = $(screen).width() - screenPad;
    ...
    this.updateDimensionsAsSections = function() {
        ...
        $('#anatomy-sections', display).css({ width: dm.width + 'px', left: (-dm.width * 0.5) + 'px' });
        this.updateSectionDimensions();
        $('#anatomy-sections', display).css({ top: -dm.height * 0.5 + 'px' });
    };
    this.updateDimensionsAsViews = function() {
        dm.width = Math.min(dm.availableWidth, parseInt($('#anatomy-views', display).css('max-width'), 10));
        $('#anatomy-views', display).css({ width: dm.width + 'px', left: (-dm.width * 0.5) + 'px' });
        dm.height = clamp(dm.availableHeight, $('#anatomy-views #anatomy-views-display', display).height(), 900) - 50;
        $('#anatomy-views', display).css({ top: -$('#anatomy-views #anatomy-views-display', display).height() * 0.5 + 'px' });
    };
    this.updateSectionDimensions = function() {
        ...
        $('.an-section .an-section-container', display).css({ width: anatomy.sectionSize + 'px', height: anatomy.sectionSize + 'px', top: Math.round((anatomy.realSectionHeight * 0.5) - (anatomy.sectionSize * 0.5)) + 'px', left: Math.round((anatomy.realSectionWidth * 0.5) - (anatomy.sectionSize * 0.5)) + 'px' });
        ...
        $('.an-section .an-section-container', display).each(function() {
            $('.section-seg', this).each(function(i) { $('.section-seg-overlay img', this).css({ top: -(anatomy.segmentSize * i) + 'px' }); });
        });
    };
    this.getNumColumns = function() { anatomy.tmpColumns = anatomy.maxColumns; while (anatomy.tmpColumns * anatomy.sectionMinSize > dm.width && anatomy.tmpColumns > 1) { anatomy.tmpColumns--; }; return anatomy.tmpColumns; };
};
...
function AnatomyWork(target, display, screen, topLevel, json, path, imageList) {
    ...
    this.createHands = function() {
        tmp = [];
        tmp.str = '';
        $('.hand-display', display).css({ width: json.hands[0].width + 'px', height: json.hands[0].height + 'px' });
        ...
        $('.hand-display .hand-element', display).smartAnimate({ opacity: 0 }, 0);
    ...
    this.setStatement = function(id) {
        ...
        if (id < 0) id = numStatements - 1;
        if (id > numStatements - 1) id = 0;
        this.hideStatement(currentStatement, function() { that.addStatement(id); });
        currentStatement = id;
    };
    this.addStatement = function(id) {
        $('.hand-display .hand-element:eq(' + id + ')', display).smartAnimate({ opacity: 1 }, 0);
    ...
    this.hideStatement = function(id, callback) {
        $('.hand-statements .hand-statement:eq(' + id + ')', display).smartAnimate(getTransition('-500px', 0), 500, function() {
            ...
            $('.hand-display .hand-element:eq(' + id + ')', display).smartAnimate({ opacity: 0 }, 300);
            callback();
        });
    };
    this.add = function() {
...
function AnatomyCore(target, display, screen, topLevel, json) {
    var target = target,
        ...
        json = json;
    this.display = display;
    this.add = function() { $(display).show(); };
    this.removeDisplay = function() {};
};
...
function AnatomyLove(target, display, screen, topLevel, json, path, imageList) {
    var target = target,
        ...
        path = path,
        tmp = [],
        cells = { cell: [], numInView: 0, slots: [] };
    currentCell = -1;
    var autoSlide = { timer: null, time: 3000 };
        ...
        if ($('.an-view-data .sec', display).length > 0) {
            while (cells.numInView < 9) {
                $('.an-view-data .sec', display).each(function(i) {
                    ...
                    cells.cell.push({ id: cells.numInView, title: $('.s-title', this).text(), timeout: null });
                    cells.numInView++;
                });
            }
        }
    ...
    this.beginAutoSlide = function() { this.autoSlide(); };
    this.autoSlide = function() { clearTimeout(autoSlide.timer); if ($(display).is(':visible')) autoSlide.timer = setTimeout(function() { that.setCenter(currentCell - 1, true); }, autoSlide.time); };
    this.stopAutoSlide = function() { clearTimeout(autoSlide.timer); };
    this.cycleCells = function(targetCell) {
        if (Math.abs(targetCell - currentCell) > Math.floor(cells.cell.length * 0.5)) { if ((targetCell - currentCell) < 0) { while (currentCell != targetCell) this.setCenter(currentCell + 1, true); } else { while (currentCell != targetCell) this.setCenter(currentCell - 1, true); } } else { if ((targetCell - currentCell) < 0) { while (currentCell != targetCell) this.setCenter(currentCell - 1, true); } else { while (currentCell != targetCell) this.setCenter(currentCell + 1, true); } }
    };
    this.setCenter = function(id, animated) {
        if (id > cells.cell.length - 1) id = 0;
        if (id < 0) id = cells.cell.length - 1;
        currentCell = id;
        cells.set = [];
    ...
    this.updateCellPosition = function(i, delay, animated) {
        ...
        if (isNaN(tmp.diff)) tmp.diff = 100;
    ...
    this.removeDisplay = function() { this.stopAutoSlide(); };
    this.readLinkData = function(data) {};
    this.init();
};
...
function ScreenWorkPlay(app, manager, display, bgColor) {
    var that = this,
        ...
        status = { adding: false, removing: false, enabled: false },
        dm = { availableWidth: 0, availableHeight: 0 },
        list = { size: 263, time: 400, closeTime: 0, closeTimeout: null },
        listHeights = [],
        iScroll = typeof(iScroll) == undefined ? undefined : window.iScroll,
        ...
        currentView = 'listing',
        clicksEnabled = true;
    textTimeout = null, gallery = { active: false, adjustTimeout: null, position: 0, image: { width: 510, height: 370, ratio: 0, realWidth: 0, realHeight: 0, div: '', item: null }, thumb: { width: 110, height: 80, div: '', item: null }, listWidth: 0, list: [], currentItem: 0, forward: true }, viewer = { currentItem: 0, forward: true };
    var imageList = { string: '' },
        preloadImgs = [{ std: '/images/screens/workplay/listview/1x/edges-a.png', retina: '/images/screens/workplay/listview/2x/edges-a.png' }, { std: '/images/screens/workplay/listview/1x/edges-b.png', retina: '/images/screens/workplay/listview/2x/edges-b.png' }, { std: '/images/screens/workplay/listview/1x/inner-border.png', retina: '/images/screens/workplay/listview/2x/inner-border.png' }, { std: '/images/screens/workplay/itemview/1x/edges-a.png', retina: '/images/screens/workplay/itemview/2x/edges-a.png' }, { std: '/images/screens/workplay/itemview/1x/edges-b.png', retina: '/images/screens/workplay/itemview/2x/edges-b.png' }, { std: '/images/screens/workplay/itemview/1x/edges-a2.png', retina: '/images/screens/workplay/itemview/2x/edges-a2.png' }, { std: '/images/screens/workplay/itemview/1x/edges-b2.png', retina: '/images/screens/workplay/itemview/2x/edges-b2.png' }, { std: '/images/screens/workplay/itemview/1x/base.png', retina: '/images/screens/workplay/itemview/2x/base.png' }];
    this.init = function() {
        this.compileImages();
        this.createListing();
        this.addListeners();
    ...
    this.createListing = function() {
        $('.wp-data .sec', display).each(function(i) {
            tmp = [];
            if ($('.s-is-info', this).text() == 'true') { tmp.display = $('.wp-templates .wp-item-info-template', display).children().clone(); } else {
            ...
            listing.push({ item: tmp.display, images: $('.s-img', this), title: $('.s-title', this).html(), content: $('.s-content', this).html(), timeout: null });
            viewerAssoc[$('.s-url', this).text().split('/')[2]] = i;
            if ($('.s-is-info', this).text() != 'true') { viewerListing.push({ item: tmp.display, images: $('.s-img', this), url: $('.s-url', this).text(), title: $('.s-title', this).html(), content: $('.s-content', this).html(), launchURL: $('.s-launch-url', this).text(), timeout: null }); }
        });
        if (iScroll != undefined && device.touch && !device.mobile) { scrollView = new Scroller($('.wp-listing-scroll', display), { momentum: true, vScrollbar: true, vScroll: true }, this.scrollStart, this.scrollEnd); }
    };
    this.scrollStart = function() { clicksEnabled = false; };
    this.scrollEnd = function() { clicksEnabled = true; };
    this.addListeners = function() {
        ...
        if (device.touch && !device.mobile) $('.wp-listing .wp-item-go .flp-btn', display).data('disabled', true);
        $('.wp-listing .wp-item-go .flp-btn', display).data({ outDisabled: true });
        $('.wp-listing .wp-item-go .wp-item-display', display).click(function() { if (!clicksEnabled) return false; });
        $('.wp-viewer .close-view', display).click(function() { trk('Work and play', 'Portfolio ' + viewerListing[viewer.currentItem].title, 'Close'); return false; });
        $('.wp-viewer', display).on(device.over, '.wp-g-thumb', function() { that.galleryThumbOver($(this).index()); }).on(device.out, '.wp-g-thumb', function() { that.galleryThumbOut($(this).index()); }).on('click', '.wp-g-thumb', function() {
        ...
        $('.wp-viewer .previous-item', display).address().click(function() { trk('Work and play', 'Portfolio ' + viewerListing[viewer.currentItem].title, 'Previous'); });
        $('.wp-viewer .next-item', display).address().click(function() { trk('Work and play', 'Portfolio ' + viewerListing[viewer.currentItem].title, 'Next'); });
        $('.wp-view-info a', display).click(function() { trk('Work and play', 'Portfolio ' + viewerListing[viewer.currentItem].title, 'Launch link ' + $(this).attr('href')) });
    };
    this.itemOver = function(e) { if (clicksEnabled) { $('.flp-btn', this).data({ outDisabled: true }); if (!$(e.target).is('.flp-btn')) { $(this).find(".flp-btn").trigger(device.over); } } };
    this.itemOut = function(e) { if (clicksEnabled) { $('.flp-btn', this).data({ outDisabled: false }); if (!$(e.target).is('.flp-btn')) { $(this).find(".flp-btn").trigger(device.out); } } };
    this.addScrollers = function() { if (!device.mobile) contentScroller = new Scroller($('.wp-view-content', display), {}); };
    this.loadAssets = function(linkData) {
        if (loadStatus.loading) return;
        loadStatus.loading = true;
        $('.sec-loader').show().find('.loading-bar-gauge').css({ width: '0%' });
        $('.wp-listing-elements img', display).smartAnimate({ opacity: 0 }).loadIndividualImages(function(item) { $(item).smartAnimate({ opacity: 1 }, 1000); });
        $('img', $('<div>' + imageList.string + '</div>')).loadImages(function() {
            loadStatus.loaded = true;
            if (!status.removing) {
    ...
    this.beginDisplay = function(linkData) {
        ...
        textTimeout = setTimeout(function() { $(window).trigger('resize.app'); }, 500);
    };
    this.resetDisplay = function() {
        clearTimeout(list.closeTimeout);
        this.clearListTimeout();
    };
    this.clearListTimeout = function() { for (var i = 0, il = listing; i < il; i++) { clearTimeout(listing[i].timeout); }; };
    this.addListing = function(reversed) {
        if (scrollView) scrollView.scrollTo(0, 0, 0, false);
        $('#container').css({ minWidth: $(display).css('min-width') });
        this.resetDisplay();
        currentView = 'listing';
    ...
    this.showGrid = function(reversed) {
        ...
        list.delay = 0;
        $('.wp-listing .wp-item', display).each(function(i) { that.showItem($(this), i, list.delay, reversed); if ((i + 1) % list.numColumns == 0) list.delay += 200; });
    };
    this.hideGrid = function(callback) {
        list.numColumns = this.getNumColumns();
        list.center = ($('.wp-listing', display).width() * 0.5) + $('.wp-listing', display).offset().left;
        $('.wp-listing .wp-item', display).each(function(i) { that.hideItem($(this), i, list, false); });
        clearTimeout(list.closeTimeout);
        list.closeTimeout = setTimeout(function() { callback(); }, list.time);
    };
    this.readLinkData = function(linkData) {
        if (linkData != undefined) {
            if (linkData.length > 1 && viewerAssoc[linkData[1]] != undefined) {
                if (currentView == "listing") { that.addItemView(viewerAssoc[linkData[1]]); } else { this.setViewerItem(viewerAssoc[linkData[1]]); }
            } else { currentView == "listing" ? this.addListing() : this.closeViewer(); }
        } else { currentView == "listing" ? this.addListing() : this.closeViewer(); };
    };
    this.addItemView = function(id) {
        if (id > viewerListing.length - 1) id = 0;
        if (id < 0) id = viewerListing.length - 1;
    ...
    this.setViewerItem = function(id) {
        ...
        if (id > viewerListing.length - 1) id = 0;
        if (id < 0) id = viewerListing.length - 1;
        if (viewer.currentItem == id) return false;
        $('.wp-view-container', display).smartAnimate(getTransition(viewer.forward ? '-110%' : '110%', 0), 500, function() {
            ...
            $('.wp-view-container', display).smartAnimate(getTransition(viewer.forward ? '110%' : '-110%', 0), 0, function() {
                ...
                if (!device.mobile) contentScroller.scrollTo(0, 0);
            });
        });
    };
    this.populateViewer = function(id) {
        ...
        if (vdata.launchURL != '') { $('.wp-launch', display).show().find('.launch-btn').attr('href', vdata.launchURL); } else { $('.wp-launch', display).hide().find('.launch-btn').attr('href', '#'); }
    ...
    this.populateGallery = function(images) {
        ...
        $.each(images, function(i) {
            ...
            gallery.list.push({ image: gallery.image.item, thumb: gallery.thumb.item });
        });
    ...
    this.setGalleryImage = function(id, instant) {
        if (!gallery.list.length) return false;
        if (id < 0) id = gallery.list.length - 1;
        id = id % gallery.list.length;
        gallery.forward = id > gallery.currentItem;
        if (id == gallery.currentItem) return;
        $('.wp-viewer .wp-g-thumb', display).removeClass('active').css({ opacity: 0.5 });
        gallery.list[id].thumb.addClass('active').smartAnimate({ opacity: 1 }, instant ? 0 : 200);
        gallery.list[id].image.show().smartAnimate($.extend(getTransition(gallery.forward ? '100%' : '-100%', 0), { opacity: 1 }), 0, function() { gallery.list[id].image.smartAnimate(getTransition('0%', 0), instant ? 0 : 300); });
        $('.wp-viewer .wp-g-img', display).append(gallery.list[id].image);
        if (gallery.currentItem > -1) { gallery.list[gallery.currentItem].image.smartAnimate({ opacity: 0.5 }, 350, function(tgt) { $(tgt).hide(); }); };
        gallery.currentItem = id;
        this.updateGalleryListPositions();
    };
    this.galleryThumbOver = function(id) {
        $('.wp-viewer .wp-g-thumb:eq(' + id + ')', display).addClass('hover').smartAnimate({ opacity: 1 }, 200);
        $('.wp-viewer .wp-g-thumb:not(:eq(' + id + '))', display).each(function() { if (!$(this).hasClass('active')) $(this).removeClass('hover').smartAnimate({ opacity: 0.5 }, 0); });
    };
    this.galleryThumbOut = function(id) { $('.wp-viewer .wp-g-thumb:eq(' + id + ')', display).removeClass('hover'); if (!$('.wp-viewer .wp-g-thumb:eq(' + id + ')', display).hasClass('active')) { $('.wp-viewer .wp-g-thumb:eq(' + id + ')', display).smartAnimate({ opacity: 0.5 }, 200); }; };
    this.showViewer = function() {
        ...
        $('.wp-view-display', display).smartAnimate(getTransition(0, ($(window).height() + 60) + 'px'), 0, function() {
            ...
            $('.wp-view .wp-view-gallery img', display).smartAnimate({ opacity: 0 }).loadIndividualImages(function(item) { $(item).smartAnimate({ opacity: 1 }, 1000); });
            gallery.active = true;
            $(window).trigger('resize.app');
            that.updateGalleryListPositions();
            if (!device.mobile) contentScroller.scrollTo(0, 0);
    ...
    this.closeViewer = function() {
        $('.wp-view-display', display).smartAnimate(getTransition(0, ($(window).height() + 60) - $('.wp-viewer', display).offset().top + 'px'), 400, function() { that.addListing(true); });
    };
    this.showItem = function(item, i, delay, reversed) {
        ...
        $('.wp-item-display', item).show().smartAnimate($.extend(getTransition(listing[i].x + 'px', reversed ? '-200px' : '200px'), { opacity: 0 }), 0, function() {
            clearTimeout(listing[i].timeout);
            listing[i].timeout = setTimeout(function() {
                $('.wp-item-display', item).smartAnimate($.extend(getTransition('0px', '0px'), { opacity: 1 }), list.time);
                $(window).trigger('resize.app');
            }, reversed ? 0 : delay);
        });
    };
    this.hideItem = function(item, i, forward, reversed) {
        listing[i].x = $(item).offset().left + ($(item).width() * 0.5) - list.center;
        listing[i].y = $(item).offset().top;
        $('.wp-item-display', item).smartAnimate($.extend(getTransition(listing[i].x + 'px', reversed == true ? '400px' : '-400px'), { opacity: 0 }), list.time, function(tgt) { $(tgt).hide(); });
    };
    this.getNumColumns = function() { list.columns = 0; while ((list.columns + 1) * list.size < $('.wp-listing', display).width()) list.columns++; return list.columns; };
    this.updateDimensions = function() {
        dm.availableWidth = $(display).width();
        dm.availableHeight = $(window).height() - $('#header').outerHeight() - 80;
        if (currentView == 'listing') { this.updateListingDimensions(); } else {
            this.updateViewerDimensions();
            clearTimeout(gallery.adjustTimeout);
            gallery.adjustTimeout = setTimeout(function() { that.updateGalleryListPositions(); }, 200);
        };
    };
    this.updateListingDimensions = function() {
        ...
        $('.wp-listing', display).css({ left: (dm.availableWidth * 0.5) - ($('.wp-listing', display).width() * 0.5), top: -dm.height * 0.5 + 'px' });
        $(display).css({ height: Math.max($(window).height(), dm.height + $('#header').outerHeight(true) + 80) });
    };
    this.updateListItemDimensions = function() {
        listHeight = [];
        $('.wp-item .wp-item-content', display).each(function() { listHeight.push($(this).outerHeight(true)); });
    ...
    this.updateViewerDimensions = function() {
        ...
        $('.wp-viewer', display).css({ left: (dm.availableWidth * 0.5) - ($('.wp-viewer', display).width() * 0.5), top: -dm.height * 0.5 + 'px' });
        $(display).css({ height: Math.max($(window).height(), dm.height + $('#header').outerHeight(true) + 80) });
        ...
        if (!device.mobile) contentScroller.refresh();
    };
    this.updateGalleryDimensions = function() {
        ...
        $('.wp-viewer .wp-g-img', display).css({ height: gallery.image.realHeight + 'px' });
        this.updateGalleryListPositions();
    };
    this.updateGalleryListPositions = function() {
        if (!gallery.active || !gallery.list.length) return false;
        ...
        if ($('.wp-g-list-items', display).width() >= $('.wp-g-list-scroll', display).width()) gallery.position = 0;
        $('.wp-g-list-scroll', display).smartAnimate(getTransition(gallery.position + 'px', 0), 300);
    };
    this.removeDisplay = function(callback) { if (currentView == 'listing') { this.removeListing(callback); } else { this.removeViewer(callback); }; };
    this.removeListing = function(callback) {
        list.numColumns = this.getNumColumns();
        list.center = ($('.wp-listing', display).width() * 0.5) + $('.wp-listing', display).offset().left;
        list.forward = true;
        $('.wp-listing .wp-item', display).each(function(i) { that.hideItem($(this), i, list.forward, true); if ((i + 1) % list.numColumns == 0) { list.forward = !list.forward; } });
        clearTimeout(list.closeTimeout);
        list.closeTimeout = setTimeout(function() { callback(); }, list.time);
    };
    this.removeViewer = function(callback) { $('.wp-viewer .wp-view-display', display).smartAnimate(getTransition(0, '200%'), 400, function() { callback(); }); };
    this.add = function(linkData) {
        manager.setBackgroundColor(bgColor);
        status.adding = true;
        if (!loadStatus.loaded) { this.loadAssets(linkData); } else { this.beginDisplay(linkData); }
    };
    this.remove = function(callback) {
        status.enabled = false;
        status.removing = true;
...
function ScreenContact(app, manager, display) {
    var that = this,
        ...
        validated = false;
    var imageList = { string: '' },
        preloadImgs = [{ std: '/images/screens/contact/dot-edge.png', retina: '/images/screens/contact/dot-edge@2x.png' }, { std: '/images/screens/contact/segments/top-left.png', retina: '/images/screens/contact/segments/top-left@2x.png' }, { std: '/images/screens/contact/segments/top-right.png', retina: '/images/screens/contact/segments/top-right@2x.png' }, { std: '/images/screens/contact/segments/bottom-left.png', retina: '/images/screens/contact/segments/bottom-left@2x.png' }, { std: '/images/screens/contact/segments/bottom-right.png', retina: '/images/screens/contact/segments/bottom-right@2x.png' }, { std: '/images/screens/contact/segments/top-bar.png', retina: '/images/screens/contact/segments/top-bar@2x.png' }, { std: '/images/screens/contact/segments/bottom-bar.png', retina: '/images/screens/contact/segments/bottom-bar@2x.png' }, { std: '/images/screens/contact/segments/left-bar.png', retina: '/images/screens/contact/segments/left-bar@2x.png' }, { std: '/images/screens/contact/segments/right-bar.png', retina: '/images/screens/contact/segments/right-bar@2x.png' }, { std: '/images/screens/contact/separator.png', retina: '/images/screens/contact/separator@2x.png' }, { std: '/images/screens/contact/stamp.png', retina: '/images/screens/contact/stamp@2x.png' }];
    this.init = function() {
        this.compileImages();
        this.setDefaults();
        this.addListeners();
    ...
    this.setDefaults = function() {
        ...
        $('.send-btn', display).click(function() { that.goSubmit(); return false; });
        $('.sendanother-btn', display).click(function() { that.goSendAnother(); return false; });
        $('.email-link', display).attr('href', 'mailto:adam' + '@' + 'adam' + 'hartwig' + '.co.uk').text('adam' + '@' + 'adam' + 'hartwig' + '.co.uk');
    };
    this.addListeners = function() {
        fields.push({ item: $('#name', display), validation: 'text' });
        fields.push({ item: $('#email', display), validation: 'text' });
        fields.push({ item: $('#message', display), validation: 'text' });
        $('#name', display).change(function() { that.validate(true, this, 'text'); });
        $('#email', display).change(function() { that.validate(true, this, 'email'); });
        $('#message', display).change(function() { that.validate(true, this, 'text'); });
    };
    this.resetFields = function() {};
    this.validate = function(silent, item, validation) {
        tmp.validated = this.testValidate(item, validation);
        tmp.inputVal = $(item).val();
        if (tmp.validated) { $(item).closest('.fm-block').addClass('validated').removeClass('validate-error'); return true; } else {
            $(item).closest('.fm-block').removeClass('validated').addClass('validate-error');
            if (silent)
                if ((tmp.inputVal == $(item).data('defaulttext') || tmp.inputVal == '')) $(item).closest('.fm-block').removeClass('validated').removeClass('validate-error');
            return false;
        };
    };
    ...
    this.goSubmit = function() {
        ...
        validated = true;
        $(fields).each(function() { tmp.itemValidation = that.validate(false, this.item, this.validation); if (!tmp.itemValidation) validated = false; });
        ...
        $('#subject', display).val("Website Enquiry");
        submitting = true;
        $.ajax({
            url: '/',
            type: 'post',
            ...
            success: function(html, textStatus, jqXHR) {
                if (html.match(/<title>Error<\/title>/)) { that.submitError(); } else { that.submitComplete(); }
            }
        });
        $(window).trigger('resize.app');
    };
    ...
    this.submitComplete = function() {
        ...
        $('.postcard', display).smartAnimate(getTransition($(window).width() + 'px', 0), 600, function() {
            ...
            $('.postcard-thankyou', display).show().smartAnimate(getTransition(-$(window).width() + 'px', 0), 0, function() { $('.postcard-thankyou', display).smartAnimate(getTransition('0px', 0), 600); });
            $(window).trigger('resize.app');
            that.resetForm();
        });
    };
    ...
    this.updateDimensions = function() { this.goUpdateDimensions(); };
    this.goUpdateDimensions = function() {
        ...
        $('#postcard-display', display).width($(display).width() - (device.mobile ? 20 : 40)).css({ left: -($('#postcard-display', display).width() * 0.5) + 'px', top: -($('#postcard-display', display).height() * 0.5) + 'px' });
    ...
    this.remove = function(callback) {
        status.enabled = false;
        status.removing = true;
        $('.postcard, .postcard-thankyou', display).smartAnimate(getTransition(-$(window).width() + 'px', 0), 300, function() { callback(); });
    };
    this.readLinkData = function(data) {};
    this.init();
};
...
function ScreenAwards(app, manager, display) {
    var that = this,
        ...
        currentCell = -1,
        awardItem;
    var autoSlide = { timer: null, time: 6000 };
    var imageList = { string: '' };
    this.init = function() {
        this.createImages();
        this.createCells();
    ...
    this.createImages = function() { imageList.string += '<img src="/images/screens/awards/grad-left.png" /><img src="/images/screens/awards/grad-right.png" />' };
    this.addListeners = function() {
        $('.next-item', display).click(function() {
        ...
        if ($('.awards-data .sec', display).length > 0) {
            while (cells.numInView < 13) {
                $('.awards-data .sec', display).each(function(i) {
                    ...
                    cells.cell.push({ id: cells.numInView, title: $('.s-title', this).text(), awards: tmp.awards, timeout: null });
                    cells.numInView++;
                });
            }
        }
    ...
    this.compileAwards = function(item) {
        ...
        $('.s-awards .s-award', item).each(function() { awardItem += '<div class="awd-item"><span class="awd-title">' + $('.award-title', this).text() + '</span> - <a href="' + $('.award-url', this).text() + '" target="_blank" title="Opens link in a new window">Launch</a></div>'; });
        awardItem += '</div>';
        return $(awardItem);
    };
    this.positionCells = function() {
        if (cells.cell.length == 0) return false;
        tmp.slotIncrement = (100 / (cells.numInView - 1));
        for (var i = 0, il = cells.numInView; i < il; i++) { cells.slots.push(Math.round(tmp.slotIncrement * i)); }
        cells.centerSlot = Math.floor(cells.slots.length / 2);
        this.setCenter(0, false);
    ...
    this.setCenter = function(id, animated) {
        ...
        that.autoSlide();
        if (device.touch) $(window).trigger('resize.app');
    };
    this.updateCellPosition = function(i, delay, animated) {
        tmp.diff = Math.abs(Math.abs($('.cell-el:eq(' + cells.set[i].id + ')', display).data('currentPos')) - Math.abs(cells.slots[i]));
        if (isNaN(tmp.diff)) tmp.diff = 100;
    ...
    this.updateDimensions = function() {
        ...
        $('#awards-items-display', display).width($(display).width() - (device.mobile ? 20 : 40)).css({ left: -($('#awards-items-display', display).width() * 0.5) + 'px', top: -($('#awards-items-display', display).height() * 0.5) + 'px' });
        $(display).height(dimension.height);
    };
    this.add = function() {
        manager.setBackgroundColor('#ffc300');
    ...
    this.remove = function(callback) {
        ...
        status.removing = true;
        this.stopAutoSlide();
        $('.awards', display).smartAnimate(getTransition(-$(window).width() + 'px', 0), 300, function() { callback(); });
    };
    this.readLinkData = function(data) {};
    this.init();
};
...
function ScreenLogin(app, manager, display) {
    ...
    this.addListeners = function() {
        $('.login-btn', display).click(function() { that.goSubmit(); return false; });
        $('.logout-btn', display).click(function() { that.logout(); return false; });
        $('form', display).submit(function() { that.goSubmit(); return false; });
    };
    this.goSubmit = function() {
        if (submitting) return false;
        validated = true;
        ...
        $.ajax({
            ...
            dataType: 'html',
            data: $('form', display).serialize(),
            error: function(jqXHR, textStatus, errorThrown) { that.submitError(); },
            success: function(msg) {
                if (msg.indexOf('You are now logged in.') >= 0) { that.submitComplete(); } else { that.submitError(); }
            }
        });
        $(window).trigger('resize.app');
    };
        ...
        $('.login-form', display).smartAnimate(getTransition($(window).width() + 'px', 0), 600, function() {
            ...
            $('.login-thankyou', display).show().smartAnimate(getTransition(-$(window).width() + 'px', 0), 0, function() {
                $('.login-thankyou', display).smartAnimate(getTransition('0px', 0), 600, function() {
                    ...
                    loginTimeout = setTimeout(function() { window.location.href = '/work-and-play'; }, 1000);
                });
            });
            $(window).trigger('resize.app');
        });
    ...
    this.setDefaults = function() { $('#username', display).setDefaultText('Your Username'); };
    this.loadAssets = function() {
        if (loadStatus.loading) return;
        loadStatus.loading = true;
        this.resetDisplay();
    ...
    this.resetDisplay = function() { $('.login-form', display).hide(); };
    this.beginDisplay = function() {
        if (loadStatus.loading) { loadStatus.loading = false; }
        status.enabled = true;
        $('#container').css({ minWidth: $(display).css('min-width') });
    ...
    this.updateDimensions = function() {
        ...
        $('#login-form-display', display).width($(display).width() - (device.mobile ? 20 : 40)).css({ left: -($('#login-form-display', display).width() * 0.5) + 'px', top: -($('#login-form-display', display).height() * 0.5) + 'px' });
        $(display).height(dimension.height);
    };
    this.add = function() {
        manager.setBackgroundColor('#02c9c9');
    ...
    this.remove = function(callback) {
        status.enabled = false;
        status.removing = true;
        $('.login-form, .login-thankyou, .logout-notice', display).smartAnimate(getTransition(-$(window).width() + 'px', 0), 300, function() { callback(); });
    };
    this.readLinkData = function(data) {};
    this.init();
};
...
function Screen404(app, manager, display) {
    ...
    this.updateDimensions = function() {
        ...
        $('#error-display', display).width($(display).width() - (device.mobile ? 20 : 40)).css({ left: -($('#error-display', display).width() * 0.5) + 'px', top: -($('#error-display', display).height() * 0.5) + 'px' });
        $(display).height(dimension.height);
    };
    this.add = function() {
        manager.setBackgroundColor('#333333');
    ...
    this.remove = function(callback) {
        status.enabled = false;
        status.removing = true;
        $('.error-notice', display).smartAnimate(getTransition(-$(window).width() + 'px', 0), 300, function() { callback(); });
    };
    this.readLinkData = function(data) {};
    this.init();
};